...
while principalMenu != 0:
    ...
    #Imprimir resultados
      ...
      while True:
        try:
          num-variables= int(input("Introduce el numero de variables: "))
          convertir_variables = int(num_variables)
          break
        except ValueError:
          print ("Por favor ingrese una cantidad valida de variables \n")
      FO = []
      #validacion y solicitud de datos para la introduccion de la Fo por parte de usuario
      for i in range(num_variables):
        while True:
          try:
            coef = float(input(f"Introduce el coeficiente de la variable x{i+1} de la funcion objetivo: "))
            FO.append(coef)
            covertir_coeficientes = float(FO[i])
            break
          except ValueError:
            print("Por favor ingrese correctamente los coeficientes de la funcion objetivo: \n")
      #validacion y solicitud de datos de datos para la introduccion del numero de restricciones
      while True:
        try:
          num_restricciones = int(input("Introduce el numero de restricciones: "))
          convertir_restricciones = int(num_restricciones)
          ...
          print("Por favor igrese una cantidad valida de restricciones \n")
      resLeft = []
      #validacion y solicitud de datos para la introduccion de las restricciones del problema(lado izquierdo)
else:
  print("Esta opcion no es validad, elige una opcion corecta \n")
principalMenu = int(input("Menu principal: \n 1- Maximar funcion por simplex \n 2- Minimizar por simplex \n 0- Salir \n"))
